    ...
    <AdminLayout>
        <div class="container px-4 py-8 mx-auto">
            <div class="overflow-hidden bg-white rounded-lg shadow-lg dark:bg-gray-800">
                ...
                <div
                    class="flex flex-col items-start justify-between p-4 space-y-4 border-b sm:p-6 sm:flex-row sm:items-center sm:space-y-0 dark:border-gray-700">
                    <div>
                    ...
                    <div class="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2">
                        <Link v-if="voluntarySaving.status === 'pending'"
                            :href="route('branch.voluntary-savings.edit', voluntarySaving.id)"
                        ...
                        <button v-if="voluntarySaving.status === 'pending'" @click="confirmDelete"
                ...
                <div class="p-4 sm:p-6">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="space-y-4">
                            <div class="p-4 border rounded-md dark:border-gray-700">
                                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-300">Member Information
                                </h3>
                                <div class="grid grid-cols-1 gap-3 mt-3">
                                    <div>
                                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400">Member
                                            Name</label>
                                        <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{
                                            voluntarySaving.member_name }}</p>
                                    </div>
                                    <div>
                                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400">Member
                                            Code</label>
                                        <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{
                                            voluntarySaving.member_code }}</p>
                                    </div>
                                    <div>
                                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400">Member
                                            Mobile</label>
                                        <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{
                                            voluntarySaving.member_mobile || 'N/A' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div class="p-4 border rounded-md dark:border-gray-700">
                                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-300">Somiti Information
                                </h3>
                                <div class="grid grid-cols-1 gap-3 mt-3">
                                    <div>
                                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400">Somiti
                                            Name</label>
                                        <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{
                                            voluntarySaving.somiti_name }}</p>
                                    </div>
                                    <div>
                                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400">Somiti
                                            Code</label>
                                        <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{
                                            voluntarySaving.somiti_code }}</p>
                                    </div>
                                    <div>
                                        <label
                                            class="text-xs font-medium text-gray-500 dark:text-gray-400">Branch</label>
                                        <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{
                                            voluntarySaving.branch.branch_name }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <div class="p-4 border rounded-md dark:border-gray-700">
                                <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-300">Application Details
                                </h3>
                                <div class="grid grid-cols-1 gap-3 mt-3">
                                    <div>
                                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400">Application
                                            Date</label>
                                        <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{
                                            formatDate(voluntarySaving.application_date) }}</p>
                                    </div>

                                    <div>
                                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400">Created
                                            On</label>
                                        <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{
                                            formatDate(voluntarySaving.created_at) }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="p-4 mt-4 border rounded-md dark:border-gray-700">
                        ...
                        <div class="grid grid-cols-1 gap-4 mt-3 md:grid-cols-3">
                            <div>
                                <label class="text-xs font-medium text-gray-500 dark:text-gray-400">Applicant
                                    Name</label>
                                <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{
                                    voluntarySaving.applicant_name }}</p>
                            </div>
                            <div>
                                <label class="text-xs font-medium text-gray-500 dark:text-gray-400">Designation</label>
                                <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{ voluntarySaving.designation
                                    || 'N/A' }}</p>
                            </div>
                            <div>
                                <label class="text-xs font-medium text-gray-500 dark:text-gray-400">PIN</label>
                                <p class="mt-1 text-sm text-gray-800 dark:text-gray-300">{{ voluntarySaving.pin || 'N/A'
                                    }}</p>
                            </div>
                        </div>
                        ...
                        <div v-if="voluntarySaving.signature" class="mt-3">
                            ...
                            <div class="mt-1">
                                <img :src="`/storage/${voluntarySaving.signature}`" alt="Applicant Signature"
                                    class="object-contain h-16 border border-gray-200 rounded-md dark:border-gray-700">
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-900">
                                    <tr>
                                        ...
                                        <th
                                            class="px-4 py-3 text-xs font-medium tracking-wider text-right text-gray-500 uppercase dark:text-gray-300">
                                            Amount
                                        </th>
                                        <th
                                            class="px-4 py-3 text-xs font-medium tracking-wider text-right text-gray-500 uppercase dark:text-gray-300">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                    <tr v-for="(deposit, index) in voluntarySaving.deposits" :key="deposit.id"
                                        class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td
                                            class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap dark:text-gray-100">
                                            {{ index + 1 }}
                                        </td>
                                        <td
                                            class="px-4 py-3 text-sm text-gray-900 whitespace-nowrap dark:text-gray-100">
                                        ...
                                        <td
                                            class="px-4 py-3 text-sm text-right text-gray-900 whitespace-nowrap dark:text-gray-100">
        ...
        <ConfirmationDialog :show="isDialogVisible" @update:show="isDialogVisible = $event"
            @confirm="deleteVoluntarySaving" title="Delete Voluntary Saving"
